...
function findAllPSiblings(where) {
    ...
    var els = where.querySelectorAll('p');

    for (var i = 0; i < els.length; i++) {

        arr.push(els[i].previousElementSibling);
    }
...
function deleteTextNodes(where) {
    ...
    for (var i = 0; i < elements.length; i++) {
        var items = elements[i];

        if (items.nodeType == Node.TEXT_NODE) {
            items.remove();
        }
    }
}
...
function deleteTextNodesRecursive(where) {
    var elements = Array.prototype.slice.call(where.childNodes);
    ...
    for (var i = 0, length = elements.length; i < length; i++) {
        ...
        if (items.childNodes.length) {
            deleteTextNodesRecursive(items);
        }
    }
}
